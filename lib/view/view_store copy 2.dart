...
class StorePage extends StatefulWidget {
  ...
  addReceip(ReceiptCategory receipt) {
    storePageLayout.addReceipcategory(receipt);
  }
  var storePageLayout = StorePageLayout();
...
class StorePageLayout extends State<StorePage> {
  ...
  List<ReceiptcardCategory> receiptTable = List();
  void addReceipcategory(ReceiptCategory _receipt) {
    setState(() {
      // debugPrint("Receipt : " + receipt.name);
      this.receiptTable.add(new ReceiptcardCategory(receipt: _receipt));
    });
  }
              ...
              child: Container(
                child: TextFieldIcon(
                    onListItemTapCategory: (ItemCategory item) {
                      ...
                      storage.addReceipcategory(new ReceiptCategory(
...
class _TextFieldIconState extends State<TextFieldIcon> {
  final ValueChanged<ItemCategory> onListItemTapCategory;
  _TextFieldIconState({this.onListItemTapCategory});
      ...
      child: Container(
        ...
        child: RawMaterialButton(
          ...
          onPressed: () {
            ...
            ket = widget.item.productcategoryId;
            debugPrint(widget.item.productcategoryId);
          },
          child: Text(
...
class ReceiptcardCategory extends StatelessWidget {
  final ReceiptCategory receipt;
  final Item item;

  const ReceiptcardCategory({Key key, this.receipt, this.item}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Text("data");
  }
